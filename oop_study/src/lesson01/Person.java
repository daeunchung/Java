...
/*
 *  # VO(Value Object) : 한 건의 데이터와 데이터를 처리할 메소드라 정의된 클래스. DTO(Data Transfer Object), bean
 *  # bean : 자바로 작업하기 위한 최소의 작업 단위로 처리할 데어터를 기억할 기억 장소.
...
public class Person {
	 ...
	 *  . 생성자는 객체가 생성되는 순간 자동으로 실행되며 멤버 변수를 최기화 시킬 목적으로 사용한다.
	 *  . 생성자를 정의하지 않으면 자바 컴파일러가 아무런 일도 하지 않는 기본 생성자를 자동으로 만든다.
	 *  . 단, 생성자를 정의하면 기본 생성자는 자동으로 만들어 지지 않는다.
	 *  . 생성자는 리턴 타입을 가지지 않고 (void도 사용하지 않는다.) return 도 사용하지 않는다.
	 * 
	 */
