...
//		Base b = new Sub();
//		자식 클래스 타입에 부모 클래스 타입의 객체가 생성된 주소를 대입했으므로 에러가 발생한다.
//		Sub s = new Base();
...
//		자식 클래스 타입에 부모 클래스 타입의 객체가 생성된 주소를 대입하면 에러가 발생되는데 이 때, casting 을 하면
//		대입은 가능하다. ==> 문법적인 에러는 발생되지 않지만 실행시켰을 때 ClassCastException 가 발생된다.
//		Sub s = (Sub) base;
