...
public class Ex15 {
	public static void main(String[] args) {
		...
		while(true) {
			...
			// 공이 가장자리에 있으면 player가 공을 밀수가 없어서
		...
		while(true) {
			...
			if(map[yy][xx] != 0 && map[yy][xx] != BALL) {continue;}
				// PLAYER or GOAL or WALL 로는 이동 불가능해서 
				// but BALL 은 만날 수 있음
			
			// 공을 만나면
			if(map[yy][xx] == BALL) {
				int yyy = ballY; int xxx = ballX;
				if(move == 1) { yyy = yyy - 1; }
				...
				// 공 이동시키기
				map[ballY][ballX] = 0; // 기존 공자리는 0으로 바꿔주고
				ballY = yyy; ballX = xxx;
